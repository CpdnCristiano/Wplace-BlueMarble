...
const wplaceColorsData = JSON.parse(
  fs.readFileSync('./wplace_colors.json', 'utf8')
);
...
testColors.forEach((testColor, index) => {
  ...
  if (result) {
    ...
    console.log(
      `   → Distância: ${result.distance.toFixed(1)} - ${matchQuality}`
    );
...
console.log(
  `🚀 Taxa: ${Math.floor(iterations / (totalTime / 1000))} conversões/segundo\n`
);
